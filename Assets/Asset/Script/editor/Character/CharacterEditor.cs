...
public class CharacterEditor : EditorWindow {
	...
	private Vector2 boxSize = new Vector2(150, 260);
	public Vector2 scrollPosition = Vector2.zero;
	private string mBase_path = "Assets/Asset/Prefab/Unit/";
	private CharacterListPrefab mCharacterInventory;

	[MenuItem ("Window/HsiN/CharacterEditor")]
	static void Init () {
		// Get existing open window or if none, make a new one:
		CharacterEditor window = (CharacterEditor)EditorWindow.GetWindow (typeof (CharacterEditor), false, "Character");
	...
	//		GUI.DrawTextureWithTexCoords(new Rect( new_position.x + 5, new_position.y+45, tr.width,tr.height), p_sprite.texture, r, true);
		...
		//Delete Button
        if (GUI.Button(new Rect(new_position.x+5, baseHeight + 5 +(intervalSpace * r_index), 80, 20), "Delete It")){
            Debug.Log("Delete " + p_prefab._name);
			DeleteScriptableObject( p_prefab );
		}		
	}
	...
	private void DeleteScriptableObject( CharacterPrefab p_prefab ) {
		mCharacterInventory.characterList.Remove( p_prefab );
		string pathToDelete = AssetDatabase.GetAssetPath(p_prefab);      
